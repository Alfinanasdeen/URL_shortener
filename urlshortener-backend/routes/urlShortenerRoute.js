...
router.post("/shortUrls", async (req, res) => {
  ...
  try {
    const short = generateUrl(); // Generate short URL using nanoid
...
router.get("/:shortUrl", async (req, res) => {
  console.log(`Redirect Request: ${req.params.shortUrl}`); // Log redirect request
  try {
    const shortUrl = await ShortUrl.findOne({ short: req.params.shortUrl });
    console.log(`Found Short URL: ${shortUrl}`); // Log found short URL

    if (!shortUrl) {
      console.log("Short URL not found"); // Log if short URL is not found
    ...
    console.error("Error during redirection:", error); // Log error
    res.status(500).json({ status: "Error", error });
  }
});
